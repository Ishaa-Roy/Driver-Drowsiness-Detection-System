...
NOD_THRESHOLD = 15  # pixels change to consider as a head nod
...
while True:
    ...
    if len(rects) == 0:
        prev_y = None  # reset nod tracking when face not detected
    ...
    for rect in rects:
        ...
        # === HEAD NODDING ===
        nose_y = shape[30][1]  # landmark 30 is nose tip
        if prev_y is not None:
            dy = nose_y - prev_y
            if dy > NOD_THRESHOLD:
                nod_count += 1
                cv2.putText(frame, "Nodding!", (width - 250, height - 60), font, 1, (0, 165, 255), 2)
                engine.say("You're nodding off. Please be alert.")
                engine.runAndWait()
                log_event("Nod detected")
        prev_y = nose_y
